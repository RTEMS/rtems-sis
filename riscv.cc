	...
	case 0:
	  ...
	  switch (funct3)
	      ...
	      if (address & 0x3)
		{
		  ...
		  address, (uint32 *) &sregs->fsi[(rs2p << 1) + BEH], 2, &ws);
	      sregs->hold += ws;
	      if (mexc)
		{
		  sregs->trap = TRAP_SEXC;
	      ...
	      if (address & LDDM)
		{
	      ...
	      sregs->hold += ws;
	      mexc |= ms->memory_write (
		  address + 4, (uint32 *) &sregs->fsi[(rs2p << 1) + 1 - BEH],
		  2, &ws);
	      sregs->hold += ws;
	      if (mexc)
		{
	...
	case 2:
	  switch (funct3)
	      ...
	      if (address & LDDM)
		{
	      ...
	      mexc = ms->memory_write (
		  address, (uint32 *) &sregs->fsi[(rs2 << 1) + BEH], 2, &ws);
	      sregs->hold += ws;
	      mexc |= ms->memory_write (
		  address + 4, (uint32 *) &sregs->fsi[(rs2 << 1) + 1 - BEH], 2,
		  &ws);
	      sregs->hold += ws;
	      if (mexc)
		{
		...
		case 4: /* FSGX */
		  ...
		  switch (funct3)
		    ...
		    case 0: /* FSGNJ */
		      ...
		      sregs->fsi[frd + 1 - BEH] =
			  (sregs->fsi[frs1 + 1 - BEH] & 0x7fffffff) |
			  (sregs->fsi[frs2 + 1 - BEH] & 0x80000000);
		      break;
		    case 1: /* FSGNJN */
		      sregs->fsi[frd + BEH] = sregs->fsi[frs1 + BEH];
		      sregs->fsi[frd + 1 - BEH] =
...
riscv_display_registers (struct pstate *sregs)
{
  ...
  printf (" sp:  %08X  a0: %08X  s2: %08X s10: %08X\n", sregs->r[2],
	  sregs->r[10], sregs->r[18], sregs->r[26]);
  printf (" gp:  %08X  a1: %08X  s3: %08X s11: %08X\n", sregs->r[3],
	  sregs->r[11], sregs->r[19], sregs->r[27]);
  printf (" tp:  %08X  a2: %08X  s4: %08X  t3: %08X\n", sregs->r[4],
	  sregs->r[12], sregs->r[20], sregs->r[28]);
  printf (" t0:  %08X  a3: %08X  s5: %08X  t4: %08X\n", sregs->r[5],
	  sregs->r[13], sregs->r[21], sregs->r[29]);
  printf (" t1:  %08X  a4: %08X  s6: %08X  t5: %08X\n", sregs->r[6],
	  sregs->r[14], sregs->r[22], sregs->r[30]);
  printf (" t2:  %08X  a5: %08X  s7: %08X  t6: %08X\n", sregs->r[7],
	  sregs->r[15], sregs->r[23], sregs->r[31]);
}
...
mstatus: %08X\n",
	  get_csr (CSR_MTVEC, sregs), get_csr (CSR_MCAUSE, sregs),
	  get_csr (CSR_MEPC, sregs), sregs->mtval,
